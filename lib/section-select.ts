...
// tokenizer-free bigram-overlap score over each section's title + body — no
// LLM tokens and no precomputed artifact, so the .md files stay the single
// source of truth.
...
export async function selectSections(
  ...
  const scored = data.sections.map((s) => ({
    section: s,
    score: sectionScore(query, `${s.title}\n${s.body}`),
  }));

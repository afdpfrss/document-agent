...
社内ドキュメントを自然言語で検索・編集できるハイブリッド文書エージェント。v1（Markdown 検索のみ）から v2（多形式取り込み・ハイブリッド検索・チャット編集・GitHub バックエンド）へ移行中。設計方針・採用機能・やらないことリストは [`docs/v2-design.md`](docs/v2-design.md) を参照。
...
- **ハイブリッド検索**: メタデータ駆動を主、ベクトル類似度を補助とした候補抽出（Phase 4）
...
├── hybrid-search.ts        メタデータ + ベクトルのハイブリッド候補抽出（Phase 4）
...
├── generate-embeddings.mjs       ハイブリッド検索用の埋め込み生成（Phase 4）
...
> **デプロイ先での注意**: 現状 `embeddings.json` は Workers 用のビルドバンドル（`lib/generated/corpus.ts`）に同梱していないため、ファイルシステムを持たない Cloudflare Workers 上ではハイブリッド層は常にフォールバックし、メタデータ駆動検索のみで動作する。ローカル / Node 実行（`next dev` / `next start`）では完全に機能する。

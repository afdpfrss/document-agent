...
├── compose/page.tsx        文書の新規作成・編集チャット（/compose、Phase 6）
...
components/                  ChatWindow / DocViewer / DocumentReference /
                             ComposePanel / EditorPanel / UploadPanel /
                             DeleteButton / SiteHeader / SiteNav
...
`/compose` の「既存を編集」モード（または `/docs`・`/documents`・引用カードの「編集」リンク）から文書を選ぶと、Monaco DiffEditor 付きのエディタが立ち上がる。右側のチャットに「~~を修正して」と指示すると、Gemini が `{find, replace, reason}[]` を構造化出力で返し、各提案が「適用可 / 原文に見つからない / 複数箇所に一致」のステータスカードで並ぶ。`/docs`・`/documents` の「編集」リンクは `/compose?mode=edit&doc=<id>` で対象文書を直接開く。
...
- `/compose` も `/api/edit/*`・`/api/compose/*` も誰でも触れる
...
|---|---|
| `一般` | 検索 (`/`、`/api/search`) のみ |
| `編集` | 上記 + 作成・編集 UI (`/compose`)・PR 作成 (`/api/edit/*`・`/api/compose/*`) |
